...
async def upstream(ups):
    ...
    if ac_br != 'master':
        await ups.edit(
            f'**[Güncelleyici]:**` Galiba Cete botunu modifiye ettin ve kendi branşını kullanıyorsun: ({ac_br}). '
            'Bu durum güncelleyicinin kafasını karıştırıyor,'
            'Güncelleme nereden çekilecek?'
            'Lütfen Cete botunu resmi repodan kullan.`')
        repo.__del__()
        return
    ...
    if not changelog and not force_update:
        await ups.edit(
            f'\n`Botun` **tamamen güncel!** `CETE:` **{ac_br}**\n')
        repo.__del__()
        return

    if conf != "now" and not force_update:
        changelog_str = f'**{ac_br} Cete için yeni güncelleme mevcut!\n\nDeğişiklikler:**\n`{changelog}`'
