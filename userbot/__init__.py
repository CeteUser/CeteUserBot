...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/CeteUser/CeteUserBot.git")
    ...
    try:
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @CeteUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Cete açabilirsin; Kanala bak` @CeteUserBot')
            else:
            ...
            if event.query.user_id == uid and query == "@CeteUserBot":
                ...
                result = builder.article(
                    ...
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben @CeteUserBot kullanıyorum!\n\nhttps://github.com/CeteUser/CeteUserBot", len(moduller)),
                    buttons=buttons,
                    link_preview=False
                )
